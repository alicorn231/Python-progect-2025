...
class Store:
    ...
    def __init__(self, root):
        ...
        "displaying products and making buttons"
        n = 0
        for item in self.items_list:
            ...
            add_button = tk.Button(self.data_frame, text="Add:",
                command=lambda i=item.item_index: self.operate(i, "add"))
            add_button.grid(row=n, column=2)
            add_button.grid(row=n, column= 2)
            add_entry = tk.Entry(self.data_frame)
            add_entry.grid(row=n, column=3)
 
            minus_button = tk.Button(self.data_frame, text="Minus:",
                command=lambda i=item.item_index: self.operate(i, "minus"))
    ...
    def operate(self, product_no, operator):
        input_str = self.entries[product_no][operator].get()
        try:
            amount = int(input_str)
            if operator == "add":
                self.items_list[product_no].stock_level += amount
            else:
                self.items_list[product_no].stock_level -= amount
            self.entries[product_no]["stock_label"].config(text=self.items_list[product_no].stock_level)
        except ValueError:
            print("Invalid input")
